...
if 'glob' in dir():
    ...
    print("glob module is not loaded")
...
with open('/gpfs/work1/0/einf2480/global_data_Qianqian/RFLEH-LAI-hc-CO2-SSM-Vcmo-IGBP_multi.pkl', 'rb') as f:
...
def Read_LSFs_Estimated_fluxes(process, job):
    ...
    ### 1) read LAI data
    probav_files = sorted(Path("/gpfs/work1/0/einf2480/global_data_Qianqian/"+year+"/lai/").glob("*"+year[0:-2]+"01*.nc"))
    print(probav_files)
    lai_10km = []
...
#        '/gpfs/work1/0/einf2480/global_data_Qianqian/output/'+year+'-H.tif',
#        tiled=True,  # GDAL: By default striped TIFF files are created. This option can be used to force creation of tiled TIFF files.
#        windowed=True,  # rioxarray: read & write one window at a time
#    )
    export_to_nc(result_LE, '/gpfs/work1/0/einf2480/global_data_Qianqian/output/'+year+'-LE96.nc')
    export_to_nc(result_H, '/gpfs/work1/0/einf2480/global_data_Qianqian/output/'+year+'-H96.nc')
