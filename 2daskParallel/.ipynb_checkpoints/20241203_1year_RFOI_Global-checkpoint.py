...
LAI = LAI.sel(time=slice(start_time, end_time))
...
SSM = SSM.reindex(time=era5.time, method='ffill')
...
CO2 = CO2.reindex(time=era5.time, method='bfill')
...
get_ipython().run_cell_magic('time', '', "LEH_ds.to_zarr(LEH_PATH, mode='w')\n")
...
get_ipython().system(' du -h $LEH_PATH')


# In[39]:


xr.open_zarr("/gpfs/work2/0/ttse0619/qianqian/global_data_Qianqian/5output_data/global_fluxes_RFOI_2014-01_v1.3.zarr")['Rn_OI'][1].plot()
