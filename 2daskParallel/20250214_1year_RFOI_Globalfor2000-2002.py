...
DATA_DIR1 = f'/projects/0/einf2480/era5land2000_2009_2020/'
ERA5_PATH = f'{DATA_DIR1}/era5land_{year}.zarr'
LAI_PATH = f'{DATA_DIR}/{year}global/lai_v2/lai_v2_{year}.zarr'
SSM_PATH = f'{DATA_DIR}/{year}global/ssm/ssm_global_{year}.zarr'
CO2_PATH = f'{DATA_DIR}/{2003}global/co2/co2_{2003}.zarr'
...
next_times = LAI.time[LAI.time > end_time_nearest.time]

if next_times.size > 0:
    end_time_next = next_times.min()
else:
    end_time_next = None  # 如果没有更晚的时间点，则选取到最后
...
# LAI2019 = xr.open_zarr('/gpfs/work2/0/ttse0619/qianqian/global_data_Qianqian/1input_data/2019global/lai_v2/lai_v2_2019correct.zarr')
# LAI2019 = LAI2019.sel(time=slice('2019-07-10T00:00:00.000000000','2019-12-31T00:00:00.000000000'))
# LAI2019['time'] = pd.to_datetime(LAI2019['time'].values) + pd.DateOffset(years=1)
# xr.concat([LAI, LAI2019], dim="time").to_zarr("/gpfs/work2/0/ttse0619/qianqian/global_data_Qianqian/1input_data/2020global/lai_v2/lai_v2_2020wholeyear.zarr")
...
# SSM = SSM.resample(time="1D").interpolate("linear")
...
# use 2003 CO2 as 2000-2002 CO2
CO2['time'] = pd.to_datetime(CO2['time'].values) - pd.DateOffset(years=(2003-year))
CO2 = CO2.sel(time=slice(start_time, pd.to_datetime(end_time) + pd.Timedelta(days=1)))
CO2 = CO2['co2']
